...
- 支持本地目录和 Git 仓库项目录入。
- 解析语言、依赖、目录树、框架和启动方式。
- 集成 Semgrep、Bandit、Gitleaks，并始终追加 custom 扫描器兜底（已移除 Trivy 依赖）。
...
| `registry.py` | 并行调度 Semgrep、Bandit、Gitleaks、custom，保证 custom 始终执行。 |
| `semgrep_runner.py` | 调用 Semgrep 并转换结果。 |
| `bandit_runner.py` | 调用 Bandit 扫描 Python 安全问题。 |
| `gitleaks_runner.py` | 调用 Gitleaks 检测密钥泄露。 |
| `custom_rules.py` | 自定义扫描器，做窗口级污点、非注入规则、弱算法 properties 解析，并调用 Python/Java AST 分析。 |
