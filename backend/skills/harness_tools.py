...
V_MODEL_GAP = "model_gap"                      # 执行了，但当前受控模型/导入无法代表目标代码
...
def _base_result(language: str, source: str, backend: str) -> dict:
    ...
    return {
        ...
        "function_completed": False, "sanitized_exception": None,
        "model_gap": False, "missing_local_import": None,
        "sink_observer": {"available": False, "observed": False},
...
def build_selfcontained_slice_harness(func: dict, vuln_type: str) -> "str | None":
    ...
    # The slice intentionally does not import project modules.  If a source
    # imported helper is required but the observer sees no sink, a MagicMock
    # stand-in would turn a model limitation into a false no-hit.  Mark that as
    # a model gap and let the verifier try the source-mounted adapter.
    _MODELED_IMPORT_NAMES = {
        "os", "subprocess", "pickle", "cPickle", "marshal", "_pickle", "yaml", "text",
        *_TAINT_NAMES, *_DIRECT_SINK_NAMES,
    }
    unmodeled_imports = sorted({
        imp.rsplit(".", 1)[-1] for imp in (func.get("imports") or [])
        if imp.rsplit(".", 1)[-1] in referenced and imp.rsplit(".", 1)[-1] not in _MODELED_IMPORT_NAMES
    })
    ...
    for name in referenced:
        if name in _TAINT_NAMES:
            ...
            # SQLAlchemy's text() creates a statement object.  Preserve the
            # SQL-string provenance instead of collapsing it into _M(), so the
            # execute observer can distinguish query construction from bound
            # parameter values.
            ns_lines.append("_ns['text'] = _sql_text\n")
        else:
    ...
    return (
        ...
        "    return ''\n"
        "class _AaxObservedSink(Exception): pass\n"
        "class _SqlText:\n"
        "    def __init__(self, statement):\n"
        "        self.statement = str(statement)\n"
        "        self.statement_tainted = _marker in self.statement\n"
        "        self.provenance = 'sqlalchemy.text'\n"
        "    def __str__(self): return self.statement\n"
        "def _sql_text(statement, *a, **k): return _SqlText(statement)\n"
        ...
        "_taint = _Taint()\n"
        "# 未知外部依赖的受控替身。SQL execute observes only its statement\n"
        "# argument: tainted bound values are parameterization evidence, not SQLi.\n"
        ...
        "            def _danger(*a, **k):\n"
        "                if name in {'execute','executescript','executemany'}:\n"
        "                    _statement = a[0] if a else None\n"
        "                    _statement_text = str(_statement)\n"
        "                    _statement_tainted = bool(getattr(_statement, 'statement_tainted', _marker in _statement_text))\n"
        "                    try: _bound_tainted = _marker in (str(a[1:]) + str(k))\n"
        "                    except Exception: _bound_tainted = False\n"
        "                    _rec.append(('sql_execute', {'available': True, 'observed': True, 'sink': name, 'statement_tainted': _statement_tainted, 'bound_parameters_tainted': _bound_tainted, 'provenance': getattr(_statement, 'provenance', 'direct_sql'), 'statement': _statement_text[:200]}))\n"
        "                    if _statement_tainted and " + _json.dumps(sink_kind) + " != 'sqli': raise _AaxObservedSink(_statement_text)\n"
        "                    return _M(self._path+'.'+name)\n"
        "                try: _text=str(a)+str(k)\n"
        "                except Exception: _text='<arg>'\n"
        "                if _marker in _text:\n"
        "                    _rec.append(('danger:'+name, _text))\n"
        "                    raise _AaxObservedSink(_text)\n"
        "                return _M(self._path+'.'+name)\n"
        "            return _danger\n"
        "        return _M(self._path+'.'+name)\n"
        ...
        + _json.dumps(sink_name) + ", 'captured_argument': None, 'payload': None, 'function_completed': False, 'sanitized_exception': type(_e).__name__, 'model_gap': True, 'sink_observer': {'available': True, 'observed': False}, 'trigger_detail': '函数体无法在切片命名空间编译'}))\n"
        ...
        "except Exception:\n"
        "    _args = []\n"
        "_unmodeled_imports=" + _json.dumps(unmodeled_imports) + "\n"
        "_triggered=False; _cap=None; _function_completed=False; _exception=None; _model_gap=False\n"
        "if callable(_fn):\n"
        "    try:\n"
        "        _target(*_args); _function_completed=True\n"
        "    except _AaxObservedSink:\n"
        "        _exception='controlled_sink_observation'\n"
        "    except Exception as _e:\n"
        "        _exception=type(_e).__name__; _model_gap=True\n"
        "else:\n"
        "    _exception='target_not_callable'; _model_gap=True\n"
        "_sql_observer = next((_r for _kind, _r in _rec if _kind == 'sql_execute'), None)\n"
        "if _unmodeled_imports: _model_gap=True; _exception=_exception or 'unmodeled_local_import'\n"
        "for _kind, _r in _rec:\n"
        "    if ((_kind == 'sink') or ("
        + _json.dumps(sink_kind) + " == 'sqli' and ((_kind == 'sql_execute' and _r.get('statement_tainted')) or _kind == 'danger:' + "
        ...
        + _json.dumps(sink_kind) + " == 'sqli' and (_kind == 'sql_execute' or _kind == 'danger:' + "
        ...
        "'function_completed': _function_completed, 'sanitized_exception': _exception, 'model_gap': _model_gap, 'missing_local_import': (_unmodeled_imports[0] if _model_gap and _unmodeled_imports else None), "
        "'sink_observer': (_sql_observer or {'available': True, 'observed': bool(_rec), 'sink': " + _json.dumps(sink_name) + "}), "
        "'trigger_detail': ('自包含切片：攻击者可控输入经真实函数逻辑到达危险 sink' if _triggered else ('模型缺口：函数在受控替身前异常' if _model_gap else '未命中 sink（可能经净化）'))}))\n"
        "print('AUDITAGENTX_VULN_TRIGGERED' if _triggered else 'AUDITAGENTX_NO_TRIGGER')\n"
    )
...
def build_local_import_adapter_harness(func: dict, vuln_type: str) -> str | None:
    """Run a real source-mounted module after a self-contained model gap.

    This deliberately small adapter supports function-level command and SQL
    sinks.  It imports the target module (therefore also its local helpers),
    then replaces only the observed sink boundary with record-only adapters.
    It is a controlled fallback, not an entrypoint claim and not a project
    profile: module and function identity come exclusively from extraction.
    """
    if not isinstance(func, dict) or not func.get("found"):
        return None
    if normalize_language(func.get("language")) != "python" or func.get("class_name"):
        return None
    module_path = (func.get("module_path") or "").replace("/", ".").replace("\\", ".").strip(".")
    fname = func.get("function_name")
    code = func.get("function_code") or ""
    if not module_path or not fname or not all(part.isidentifier() for part in module_path.split(".")):
        return None
    if re.search(r"\.(execute|executescript|executemany)\s*\(", code):
        sink_kind, sink_name = "sqli", "execute"
    elif re.search(r"os\.system|os\.popen|subprocess\.", code):
        sink_kind, sink_name = "command", "os.system"
    else:
        return None

    import json as _json
    marker = "AAXIMPORT_" + secrets.token_hex(6)
    invoke_probe = TARGET_INVOKED_MARKER + NONCE_PLACEHOLDER
    return (
        "import importlib, inspect, json, os, subprocess, sys\n"
        "sys.path.insert(0, '/target')\n"
        f"_marker={_json.dumps(marker)}; _nonce={_json.dumps(invoke_probe)}\n"
        f"_sink_kind={_json.dumps(sink_kind)}; _sink_name={_json.dumps(sink_name)}\n"
        "_rec=[]; _module=None; _import_error=None; _missing_local_import=None\n"
        "try:\n"
        f"    _module=importlib.import_module({_json.dumps(module_path)})\n"
        "except ModuleNotFoundError as _e:\n"
        "    _import_error=type(_e).__name__; _missing_local_import=getattr(_e, 'name', None)\n"
        "except Exception as _e:\n"
        "    _import_error=type(_e).__name__\n"
        "class _SqlText:\n"
        "    def __init__(self, statement): self.statement=str(statement); self.statement_tainted=_marker in self.statement; self.provenance='sqlalchemy.text'\n"
        "    def __str__(self): return self.statement\n"
        "def _text(statement, *a, **k): return _SqlText(statement)\n"
        "def _execute(statement=None, *a, **k):\n"
        "    text=str(statement); tainted=bool(getattr(statement, 'statement_tainted', _marker in text))\n"
        "    try: bound=_marker in (str(a)+str(k))\n"
        "    except Exception: bound=False\n"
        "    _rec.append({'available':True,'observed':True,'sink':_sink_name, 'statement_tainted':tainted, 'bound_parameters_tainted':bound, 'provenance':getattr(statement, 'provenance', 'direct_sql'), 'statement':text[:200]})\n"
        "    return None\n"
        "class _DbAdapter:\n"
        "    def __getattr__(self, name): return _execute if name in {'execute','executescript','executemany'} else self\n"
        "    def __call__(self, *a, **k): return self\n"
        "def _command(*a, **k): _rec.append({'sink':_sink_name, 'statement_tainted':_marker in (str(a)+str(k)), 'bound_parameters_tainted':False, 'provenance':'command_argument'}); return ''\n"
        "if _module is not None:\n"
        "    if _sink_kind == 'sqli':\n"
        "        for _name in ('db','session','connection','engine'):\n"
        "            if hasattr(_module, _name): setattr(_module, _name, _DbAdapter())\n"
        "        if hasattr(_module, 'text'): setattr(_module, 'text', _text)\n"
        "    else:\n"
        "        os.system=_command; os.popen=_command; subprocess.run=_command; subprocess.call=_command; subprocess.check_output=_command; subprocess.Popen=_command\n"
        f"_target=getattr(_module, {_json.dumps(fname)}, None) if _module is not None else None\n"
        "_completed=False; _exception=None\n"
        "if callable(_target):\n"
        "    try:\n"
        "        _args=[_marker if p.default is inspect._empty else None for p in inspect.signature(_target).parameters.values() if p.kind in (p.POSITIONAL_ONLY,p.POSITIONAL_OR_KEYWORD)]\n"
        "        print(_nonce); _target(*_args); _completed=True\n"
        "    except Exception as _e: _exception=type(_e).__name__\n"
        "else:\n"
        "    _exception='target_not_callable'\n"
        "_observer=_rec[-1] if _rec else {'available':True,'observed':False,'sink':_sink_name}\n"
        "_triggered=bool(_observer.get('statement_tainted'))\n"
        "_model_gap=bool(_import_error or (_exception and not _rec))\n"
        "print('AUDITAGENTX_RESULT_JSON='+json.dumps({'triggered':_triggered,'sink_called':bool(_rec),'sink_name':_sink_name,'captured_argument':_observer.get('statement'),'payload':(_marker if _triggered else None),'function_completed':_completed,'sanitized_exception':(_import_error or _exception),'model_gap':_model_gap,'missing_local_import':_missing_local_import,'sink_observer':_observer,'trigger_detail':('source-mounted module/helper adapter observed tainted sink argument' if _triggered else ('local import/model gap' if _model_gap else 'executed sink observer no-hit'))}))\n"
        "print('AUDITAGENTX_VULN_TRIGGERED' if _triggered else 'AUDITAGENTX_NO_TRIGGER')\n"
    )
...
def _finalize(exec_out: dict, source: str, language: str, backend: str,
    ...
    if parsed:
        ...
        res["function_completed"] = bool(parsed.get("function_completed"))
        raw_exception = str(parsed.get("sanitized_exception") or "")
        # Harness evidence may cross project boundaries.  Keep only a compact
        # exception class/message token; never retain paths, arguments, or
        # traceback data from the mounted source.
        res["sanitized_exception"] = re.sub(r"[^A-Za-z0-9_.:-]", "_", raw_exception)[:120] or None
        res["model_gap"] = bool(parsed.get("model_gap"))
        missing_local_import = parsed.get("missing_local_import")
        res["missing_local_import"] = (
            re.sub(r"[^A-Za-z0-9_.-]", "_", str(missing_local_import))[:120]
            if missing_local_import else None
        )
        observer = parsed.get("sink_observer")
        if isinstance(observer, dict):
            res["sink_observer"] = {
                key: observer.get(key) for key in (
                    "available", "observed", "sink", "statement_tainted",
                    "bound_parameters_tainted", "provenance",
                ) if key in observer
            }
        import_error = str(parsed.get("import_error") or "")
        if import_error:
            # An import failure means the source/model could not be represented.
            # It is never evidence that an executed target was a clean no-hit.
            res["model_gap"] = True
            res["sanitized_exception"] = res["sanitized_exception"] or "import_error"
    else:
        # 退回旧 marker
    ...
    if not res["triggered"]:
        if res["model_gap"]:
            res["verdict"] = V_MODEL_GAP
            res["reason"] = res["reason"] or "model_gap: controlled harness could not represent target execution"
        elif exec_out.get("timed_out"):
